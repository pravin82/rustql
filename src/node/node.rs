...
use crate::table::table::{PAGE_SIZE, Table};
...
const COMMON_NODE_HEADER_SIZE: u32 =
    (NODE_TYPE_SIZE + IS_ROOT_SIZE + PARENT_POINTER_SIZE) as u32;
...
const LEAF_NODE_HEADER_SIZE: u32 =
    COMMON_NODE_HEADER_SIZE as u32 + LEAF_NODE_NUM_CELLS_SIZE;
...
unsafe fn get_leaf_node_cell_ptr(node_ptr:*const u8,cell_num:u32)->*mut u8{
       node_ptr
           ...
           .add((cell_num * LEAF_NODE_CELL_SIZE) as usize).cast_mut()
}
...
pub unsafe fn leaf_node_insert(
    cursor:Cursor,
    key:u32,
    value:Row,
    mut table: &mut Table,
)->Result<String,Error>{
    let node_ptr = table.pager.get_page(cursor.page_num).unwrap();
...
pub unsafe fn find_key_in_leaf_node(table: &mut Table, page_num:u32, key:u32)->Cursor{
    ...
    while(one_past_max_index != min_index){
        ...
        if(key > key_at_index) {min_index = index+1}
    ...
    let cursor = Cursor{page_num,cell_num:required_index, end_of_table: false };
    return cursor
